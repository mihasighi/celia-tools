                     ...
                     ap_dimchange_t * dimchange, bool project)
{
  ...
  for (i = 0; i < addr; i++)
    {
      arg_assert (dimchange->dim[addi + i] == (a->datadim + a->segmdim),
                  return NULL;);
                        ...
                        ap_dimchange_t * dimchange)
{
  ...
  lsum_t *r = lsum_alloc_internal (pr, a->datadim - dimchange->intdim,
                                   a->segmdim - dimchange->realdim);
  size_t i;
  ...
  for (i = 0; i < dimchange->realdim; i++)
    {
      dimrm.dim[dimchange->intdim + 3 * i + 0] =
        dimchange->dim[dimchange->intdim + i];
      dimrm.dim[dimchange->intdim + 3 * i + 1] =
        a->segmdim + dimchange->dim[dimchange->intdim + i];
      dimrm.dim[dimchange->intdim + 3 * i + 2] =
        2 * a->segmdim + dimchange->dim[dimchange->intdim + i];
    }
...
/**
 * @brief Make array @p dim a singleton.
 *
 * The input @p a is constrained to have array @p dim of length 1.
 */
lsum_t *
...
/**
 * @brief Split array @p dim on its head and its tail.
 *
 * Add a new dimension to the input @p a representing the tail
 * of the array @p dim.
 */
lsum_t *
...
/**
 * @brief Split array @p dim on two arrays, the first of length @p idim.
 *
 */
lsum_t *
             ...
             bool destructive, lsum_t * a, ap_dim_t dim, size_t n)
{
  ...
  if (!a)
    return NULL;
  arg_assert (n <= (3 + a->datadim) && dim > 0
              && dim < a->segmdim, return NULL;);
  lsum_t *r;
  switch (n)
    {
    case 1:
