...
#ifndef NDEBUG1
	fprintf(stdout,"  \n\t apply_permutation: \n");
	for(size_t tmpc = 0; tmpc<perm->size; tmpc++)
		fprintf(stdout," %zu", perm->dim[tmpc]);
...
ucons_expand (ap_manager_t * man,
	...
	/*  Careful cause cond 3 not dealed with !!!!
	 *   is the general case for splitting */
	ucons_t *r;
	if (n == 1)
