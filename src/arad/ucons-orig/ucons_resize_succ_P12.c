	...
	if(jdcons){
		...
		if(jdcons->dcons!=NULL){
			...
			if(!test_l_leq_v(pr->man_dcons, r->econs,
				...
				//ap_abstract0_t * temp_y1 = ap_abstract0_copy(pr->man_dcons, ap_jdcons_y1);
...
// and  y = l[n2] -1 meet y = 1 if len(n1) <= 3
				...
				if(test_l_leq_v(pr->man_dcons, r->econs,
					...
					/****************  intersect with temp_y1 the constraint with pattern_1_l1 *************************************************/

					//add dims y2 dy2 to temp_y1
					// forget the value of y1
					//
