...
ucons_t * fold_with_closure_P11_or_P12(ucons_internal_t *pr, ucons_t * a,
		ap_dim_t * tdim, size_t size, bool update_lenght) {
	...
	for (i = 0; i < size; i++) {
		if (tdim[i] >= a->datadim)
			size_node_tdim++;
		checked_realloc(node_tdim, ap_dim_t, sizeof(ap_dim_t), size_node_tdim,
				return NULL;);
	...
	while (i < size_node_tdim) {
		...
		if (ap_abstract0_sat_lincons(pr->man_dcons, r->econs, &cons)) {
			...
			if (i - start == 0) {
				checked_realloc(cdim, ap_dim_t, sizeof(ap_dim_t), (size_cdim+1),
						return NULL;);
				...
				checked_realloc(cdim, ap_dim_t, sizeof(ap_dim_t), (size_cdim+2),
						return NULL;);
				cdim[size_cdim] = node_tdim[start];
				...
				size_gdim = i - start;
				checked_malloc(gdim, ap_dim_t, sizeof(ap_dim_t), size_gdim,
						return NULL;);
...
ucons_t * split_with_pattern_P12_P11(ucons_internal_t* pr, ucons_t *r,
	...
	if (aux) {
		...
		if (!aux_find2) {
			...
			checked_malloc(aux_find2, pattern_t, 1,
		...
		checked_malloc(yexpr, ap_linexpr0_t*, nr_y, sizeof(ap_linexpr0_t*),
				return NULL;);
		...
		if (found_val != NULL)
			aux_find2->dcons = ap_abstract0_meet(pr->man_dcons, true,
					aux_find2->dcons, found_val);

		for (i = 0; i < nr_y; i++) {
			ydim[i] = r->datadim + 2 * r->segmentdim + i;

			yexpr[i] = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
					r->datadim + 2 * r->segmentdim + 2 * nr_y);
			ap_linexpr0_set_coeff_scalar_int(yexpr[i], ydim[i], 1);
			ap_linexpr0_set_cst_scalar_int(yexpr[i], 1);
			aux_find2->dcons = ap_abstract0_substitute_linexpr(pr->man_dcons,
					true, aux_find2->dcons, ydim[i], yexpr[i], NULL);
		}
		...
		if (pattern_j->type == 0) {
			...
			ap_linexpr0_t *exprrd0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
					r->datadim + 2 * r->segmentdim + 2);
			ap_linexpr0_set_coeff_scalar_int(exprrd0, dim_y1, -1);
			ap_linexpr0_set_coeff_scalar_int(exprrd0,
					n2 + r->datadim + r->segmentdim, 1);
			...
			aux_find2->dcons = ap_abstract0_meet_lincons_array(pr->man_dcons,
					true, aux_find2->dcons, &arr);
			...
			flag = ap_abstract0_sat_lincons(pr->man_dcons, aux_find2->dcons,
					&cons);
			ap_lincons0_clear(&cons);
			/* if aux_find2->dcons implies y2 == 1 */
			if (flag)
				aux_find2->dcons = ap_abstract0_bottom(pr->man_dcons,
						r->datadim + 2 * r->segmentdim + 2 * nr_y, 0);
			...
			/* impose (y2 <= ln2 - 1) */
			ap_linexpr0_t *exprrd = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
					r->datadim + 2 * r->segmentdim + 2 * nr_y);
			ap_linexpr0_set_coeff_scalar_int(exprrd, dim_y2, -1);
			ap_linexpr0_set_coeff_scalar_int(exprrd,
					n2 + r->datadim + r->segmentdim, 1);
		...
		if (pr->active_patterns[4]) {
			...
			//	test_pattern_sat(pr,r,pattern_j,0)){
				...
				temp_data = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
						temp_data, &arr);
				ap_lincons0_array_clear(&arr);
				...
				checked_malloc(tdim, ap_dim_t, 2, sizeof(ap_dim_t),
						return NULL;);
				tdim[0] = y1;
				tdim[1] = dy1;
				temp_data = ap_abstract0_forget_array(pr->man_dcons, true,
						temp_data, tdim, 1, true);
				//  remove dimension y1, dy1
				dimchange = ap_dimchange_alloc(2, 0);
				dimchange->dim[0] = y1;
				dimchange->dim[1] = dy1;
				temp_data = ap_abstract0_remove_dimensions(pr->man_dcons, true,
						temp_data, dimchange);
				...
				temp_data = ap_abstract0_substitute_linexpr(pr->man_dcons, true,
						temp_data, dim_y1, expr, NULL);
				...
				arr.p[0].linexpr0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
						r->datadim + 2 * r->segmentdim + 2);
				arr.p[0].scalar = NULL;
				ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, dim_y1, 1);
				ap_linexpr0_set_cst_scalar_int(arr.p[0].linexpr0, -1);
				...
				if (aux_find2) {
					...
					arr.p[0].linexpr0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
							r->datadim + 2 * r->segmentdim + 4);
					arr.p[0].scalar = NULL;
					ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, dim_y1,
							-1);
					ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, dim_y2,
							1);

					aux_find2->dcons = ap_abstract0_meet_lincons_array(
							pr->man_dcons, true, aux_find2->dcons, &arr);
					ap_lincons0_array_clear(&arr);

					ap_abstract0_t * temp_data_p12 = ap_abstract0_copy(
							pr->man_dcons, temp_data);
					...
					temp_data_p12 = ap_abstract0_add_dimensions(pr->man_dcons,
							false, temp_data_p12, &dimchange, false);
					free(dimchange.dim);
					...
					temp_data_p12 = ap_abstract0_meet_lincons_array(
							pr->man_dcons, true, temp_data_p12, &arr);
					ap_lincons0_array_clear(&arr);

					aux_find2->dcons = ap_abstract0_meet(pr->man_dcons, true,
							aux_find2->dcons, temp_data_p12);
				...
				if (sub_pattern_data) {
					if (sub_pattern_data->dcons != NULL) {
						sub_pattern_data->dcons = ap_abstract0_meet(
								pr->man_dcons, true, sub_pattern_data->dcons,
								temp_data);
						/*
						...
						sub_pattern_data->dcons = ap_abstract0_copy(
								pr->man_dcons, temp_data);
						/*						fprintf(stdout,"\n sub_pattern_data= null \n ");
						 fprintf(stdout,"\n");
						 fflush(stdout);
					 ...
					 */
					checked_malloc(sub_pattern_data, pattern_t, 1,
							sizeof(pattern_t)+1*sizeof(size_t), return NULL;);
					memset(sub_pattern_data, 0,
							sizeof(pattern_t) + (1) * sizeof(size_t));
					sub_pattern_data->key.type = sub_pattern->type;
					for (size_t i = 0; i < 1; i++)
						sub_pattern_data->key.segments[i] =
								sub_pattern->segments[i];
					sub_pattern_data->dcons = ap_abstract0_copy(pr->man_dcons,
							temp_data);
					ap_abstract0_free(pr->man_dcons, temp_data);
...
ucons_t * ucons_change_pattern_2_1(ucons_internal_t * pr, ucons_t *r,
		ucons_t * a, ap_dim_t * tdim, size_t size_tdim,
	...
	if (e_seg >= size_tdim) {
		...
		if (found) {
			...
			for (size_t kk = 1; kk < size_tdim && found; kk++) {
				if (!pattern_j->segments[jj] == tdim[kk])
					found = false;
				jj++;
			}
			if (found) {
				...
				checked_malloc(look, pattern_key_t,
						sizeof(pattern_key_t) + (u_seg+(e_seg-size_tdim+1))*sizeof(size_t),
						1, return NULL;);
				memset(look, 0,
						sizeof(pattern_key_t)
								+ (u_seg + (e_seg - size_tdim + 1))
										* sizeof(size_t));
				look->type = get_pattern_type(pr, u_seg,
						(e_seg - size_tdim + 1), nr_y, kind);
				unsigned keylen = (u_seg + (e_seg - size_tdim + 1))
						* sizeof(size_t) + sizeof(pattern_key_t);
				look->segments[0] = pattern_j->segments[0];
				look->segments[1] = pattern_j->segments[1];
				for (size_t jj = u_seg; jj < (u_seg + e_seg - size_tdim + 1);
						jj++) {
					if (jj <= pos_hd)
						look->segments[jj] = pattern_j->segments[jj];
					else
						look->segments[jj] =
								pattern_j->segments[jj + size_tdim];
				}
				...
				unsigned keylen_curr = (u_seg + e_seg) * sizeof(size_t)
						+ sizeof(pattern_key_t);
				HASH_FIND(hh, a->udcons, pattern_j, keylen_curr, curr);
				if (curr) {
					if (res)
						res->dcons = ap_abstract0_join(pr->man_dcons, false,
								curr->dcons, res->dcons);
					else {
						checked_malloc(res, pattern_t, 1,
								(sizeof(pattern_t)+(u_seg+(e_seg-size_tdim+1))*sizeof(size_t)),
								return NULL;);
						memset(res, 0,
								(sizeof(pattern_t)
										+ (u_seg + (e_seg - size_tdim + 1))
						...
						res->dcons = ap_abstract0_copy(pr->man_dcons,
								curr->dcons);
						res->key.type = look->type;

						for (size_t ii = 0;
								ii < (u_seg + (e_seg - size_tdim + 1)); ii++) {
...
ucons_t * ucons_fold_pattern_2_1(ucons_internal_t * pr, ucons_t *r, ucons_t * a,
		ap_dim_t * tdim, size_t size_tdim, pattern_key_t * pattern_j) {
	...
	if (!test_singleton(pr->man_dcons, a->econs, a->datadim, a->segmentdim,
			tdim[0])) {
		if (ucons_res == NULL) {
			/**/
	...
	for (ctr = 1; ctr < size_tdim; ctr++) {
		...
		checked_realloc(len_e_seg, ap_dim_t, sizeof(ap_dim_t), (le_seg + 1),
				return NULL;);
		size_t ii = le_seg;
		...
		checked_malloc(look1, pattern_key_t,
				sizeof(pattern_key_t) + (1+le_seg)*sizeof(size_t), 1,
				return NULL;);
		...
		if (!test_singleton(pr->man_dcons, r->econs, r->datadim, r->segmentdim,
				tdim[ctr])) {
			bool ordCtr = true; /*true if samme order as between tdim[0] and hn  */
			checked_malloc(look2, pattern_key_t,
					sizeof(pattern_key_t) + (u_seg+le_seg)*sizeof(size_t), 1,
					return NULL;);
			if (tdim[ctr] < hn) {
				look2->segments[0] = tdim[ctr];
				look2->segments[1] = hn;
				look2->type = get_pattern_type(pr, u_seg, le_seg, 2,
						pattern_2_1_lx);
				if (ord)
					ordCtr = true;
				else
			...
			if (!ucons2) {
				/* no constraint with pattern in closure returns top with the analyzed pattern */
				keylen = (u_seg + e_seg) * sizeof(size_t)
						+ sizeof(pattern_key_t);
				HASH_FIND(hh, r->udcons, pattern_j, keylen, ucons_res);
				if (ucons_res) {
					HASH_DEL(r->udcons, ucons_res);
			...
			if (ordCtr == false) {
				...
				consperm.dim = (ap_dim_t *) malloc(
						consperm.size * sizeof(ap_dim_t));
				ap_dimperm_set_id(&consperm);

				for (i = 0; i < u_seg; i++) {
					consperm.dim[r->datadim + 2 * r->segmentdim + 2 * i] =
							r->datadim + 2 * r->segmentdim + (2 * i) + 1;
					consperm.dim[r->datadim + 2 * r->segmentdim + 2 * i + 1] =
							r->datadim + 2 * r->segmentdim + (2 * i);
				}
				dcons2 = ap_abstract0_permute_dimensions(pr->man_dcons, false,
						ucons2->dcons, &consperm);
				free(consperm.dim);
			} else
				dcons2 = ap_abstract0_copy(pr->man_dcons, ucons2->dcons);
		} else {
			dcons2 = ap_abstract0_bottom(pr->man_dcons,
		...
		arr.p[1].linexpr0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
				r->datadim + 2 * r->segmentdim + 4);
		arr.p[1].scalar = NULL;
		ap_linexpr0_set_coeff_scalar_int(arr.p[1].linexpr0, r->datadim + dctr,
				1);
		...
		if (ord) {
			...
			dcons1 = ap_abstract0_add_dimensions(pr->man_dcons, false,
					ucons1->dcons, &dimchange, false);
		...
		dcons1 = ap_abstract0_meet_lincons_array(pr->man_dcons, true, dcons1,
				&arr);
		ap_lincons0_array_clear(&arr);
...
ucons_t * ucons_fold_pattern_1_lx(ucons_internal_t * pr, ucons_t *r,
		ucons_t * a, ap_dim_t * tdim, size_t size_tdim,
		pattern_key_t * pattern_j) {
	...
	if (pattern_j->segments[0] == hd) {
		...
		for (size_t jj = 0; jj < e_seg && !found; jj++) {
			for (size_t kk = 1; (kk < size_tdim && !found); kk++)
				found = (pattern_j->segments[jj + 1] == tdim[kk]) ?
						true : false;
		}
		if (found) {
			r = remove_pattern_n2p(pr, r, pattern_j);
			unsigned keylen = (u_seg + e_seg) * sizeof(size_t)
					+ sizeof(pattern_key_t);
			HASH_FIND(hh, r->udcons, pattern_j, keylen, aux);
			if (aux) {
				HASH_DEL(r->udcons, aux);
					...
					memset(look, 0,
							sizeof(pattern_key_t)
									+ (u_seg + (e_seg - size_tdim + 1))
											* sizeof(size_t));
					look->type = get_pattern_type(pr, u_seg,
							(e_seg - size_tdim + 1), 1, kind);
					unsigned keylen = (u_seg + (e_seg - size_tdim + 1))
							* sizeof(size_t) + sizeof(pattern_key_t);
					look->segments[0] = pattern_j->segments[0];
					for (size_t jj = u_seg;
							jj < (u_seg + e_seg - size_tdim + 1); jj++) {
						if (jj <= pos_hd)
							look->segments[jj] = pattern_j->segments[jj];
						else
							look->segments[jj] = pattern_j->segments[jj
 ...
 * minus_dim > 0
 * */
ucons_t * fold_with_closoure_of_P21(ucons_internal_t * pr, ucons_t * a,
		ap_dim_t * node_tdim, size_t size_node_tdim, ap_dim_t minus_dim,
		bool update_length) {
	...
	arg_assert(
			(update_length ||(!update_length&& ((minus_dim>0)|| (pr->active_patterns[1])))),
			return NULL;);
	...
	for (i = 0; i < size_node_tdim; i++) {
		...
		if (update_length == true
				|| (!update_length && pr->active_patterns[1])) { // PB
			// incoding of 0 2
			if (node_tdim[i] >= a->datadim) {
				size_tdim++;
				checked_realloc(tdim, ap_dim_t, sizeof(ap_dim_t),
						size_node_tdim, return NULL;);
...
#endif
			size_tdim++;
			checked_realloc(tdim, ap_dim_t, sizeof(ap_dim_t), size_node_tdim,
					return NULL;);
	...
	hd_sgl = test_singleton(pr->man_dcons, a->econs, a->datadim, a->segmentdim,
			hd);
	...
	for (j = 0; j < patterns_on_hd.size; j++) {
		...
		if (patterns_on_hd.p[j] != NULL && patterns_on_hd.p[j]->type == 1
				&& ((patterns_on_hd.p[j]->segments[0] == hd
						&& patterns_on_hd.p[j]->segments[1] == minus_dim)
						|| (patterns_on_hd.p[j]->segments[1] == hd
								&& patterns_on_hd.p[j]->segments[0] == minus_dim)))
			ignore_pattern = true;
		...
		if (patterns_on_hd.p[j] != NULL && ignore_pattern == false) {
			...
			if (pr->PI[pattern_j->type].kind == pattern_1_lx) {
				r = ucons_fold_pattern_1_lx(pr, r, a, tdim, size_tdim,
						pattern_j);
				if (hd_sgl) {
					if (pr->PI[pattern_j->type].e_seg == 1) {

						pattern_key_t *new_key;
						checked_malloc(new_key, pattern_key_t,
								sizeof(pattern_key_t) + (1+u_seg)*sizeof(size_t),
								1, return NULL;);
						memset(new_key, 0,
								sizeof(pattern_key_t)
										+ (1 + u_seg) * sizeof(size_t));
						new_key->type = get_pattern_type(pr, 2, 0, 2,
								pattern_2_1);
						...
						if (!((new_key->segments[0] == hd
								&& new_key->segments[1] == minus_dim)
								|| (new_key->segments[1] == hd
										&& new_key->segments[0] == minus_dim))) {
							...
							fflush(stdout);
#endif
							r = ucons_fold_pattern_2_1(pr, r, a, tdim,
									size_tdim, new_key);
						}
						...
						size_t pos_hd = 0;
						for (size_t ii = 1;
								(ii < 1 + pr->PI[pattern_j->type].e_seg)
										&& !ntdim; ii++) {
							if (pattern_j->segments[ii] == tdim[0])
								pos_hd = ii;
							for (size_t jj = 1; ((jj < size_tdim) && !ntdim);
									jj++)
						...
						if (!ntdim) {
							pattern_key_t *new_key;
							checked_malloc(new_key, pattern_key_t,
									sizeof(pattern_key_t) + u_seg*sizeof(size_t),
									1, return NULL;);
							memset(new_key, 0,
									sizeof(pattern_key_t)
											+ (u_seg + e_seg - 1)
													* sizeof(size_t));
							if (pattern_j->segments[0]
									< pattern_j->segments[1]) {
								new_key->segments[0] = pattern_j->segments[0];
								new_key->segments[1] = pattern_j->segments[1];
							} else {
								new_key->segments[0] = pattern_j->segments[1];
							...
							for (size_t kk = 0; kk < e_seg; kk++) {
								if (pattern_j->segments[kk] == tdim[0])
									kk++;
								else
									new_key->segments[nk++] =
											pattern_j->segments[kk];
							}
							new_key->type = get_pattern_type(pr, 2, 0, 2,
									pattern_2_1_lx);
							r = ucons_fold_pattern_2_1(pr, r, a, tdim,
			...
			if (pr->PI[pattern_j->type].kind == pattern_2_1) {
				if ((pattern_j->segments[0] == tdim[0])
						|| (pattern_j->segments[1] == tdim[0]))
					r = ucons_fold_pattern_2_1(pr, r, a, tdim, size_tdim,
							pattern_j);
				else
					r = ucons_change_pattern_2_1(pr, r, a, tdim, size_tdim,
							pattern_j);
			}
	...
	/**** !!! ***/
	/** TODO daca nu am nici un pattern generat cred ca ar fi cazul sa generez unul ! sa nu pierd tot  **/
	...
	if (update_length) {
		...
		r->econs = ap_abstract0_assign_linexpr(pr->man_dcons, true, r->econs,
				lhd, expr, NULL);
		ap_linexpr0_free(expr);
		...
		for (s = r->udcons; s != NULL; s = s->hh.next) {
			if (pr->PI[s->key.type].nr_y == 1) {
				s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true,
						s->dcons, lhd, expr_1y, NULL);
			} else {
				s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true,
						s->dcons, lhd, expr_2y, NULL);
			}
		}
...
ucons_t * fold_without_closoure_of_P21(ucons_internal_t * pr, ucons_t * a,
		ap_dim_t *fold_dim, size_t size_fold_dim, bool update_length) {
	...
	checked_realloc(fold_dim, ap_dim_t, sizeof(ap_dim_t), size_fold_dim,
			return NULL;);

	for (i = 0; i < size; i++) {
		if (tdim[i] >= r->datadim) {
			size_snd_node_tdim++;
			checked_realloc(snd_node_tdim, ap_dim_t, sizeof(ap_dim_t),
					size_snd_node_tdim, return NULL;);
	...
	for (i = 0; i < size_fold_dim; i++) {
		if (fold_dim[i] >= r->datadim) {
			size_fst_node_tdim++;
			checked_realloc(fst_node_tdim, ap_dim_t, sizeof(ap_dim_t),
					size_fst_node_tdim, return NULL;);
	 ...
	 *
	 */
	ucons_t *rr = fold_with_closoure_of_P21(pr, r, fst_node_tdim,
			size_fst_node_tdim, snd_node_tdim[0], false);
	ucons_free_internal(pr, r);
	r = NULL;
	r = rr;
	if (pr->active_patterns[1]) {
		rr = fold_with_closure_P11_or_P12(pr, r, fst_node_tdim,
				size_fst_node_tdim, false);
		ucons_free_internal(pr, r);
		r = NULL;
		r = rr;
	}
	...
	fflush(stdout);
#endif
	rr = fold_with_closoure_of_P21(pr, r, snd_node_tdim, size_snd_node_tdim,
			fst_node_tdim[0], false);
	ucons_free_internal(pr, r);
	r = NULL;
	r = rr;
	if (pr->active_patterns[1]) {
		rr = fold_with_closure_P11_or_P12(pr, r, snd_node_tdim,
				size_snd_node_tdim, false);
		ucons_free_internal(pr, r);
		r = NULL;
		r = rr;
	}
	...
	if (!test_equal_length(pr->man_dcons, r->econs, r->datadim, r->segmentdim,
			fst_node_tdim[0], snd_node_tdim[0])) {
		pattern_t *nd;
		checked_malloc(nd, pattern_t, 1, (sizeof(pattern_t)+2*sizeof(size_t)),
				return NULL;);
		memset(nd, 0, (sizeof(pattern_t) + 2 * sizeof(size_t)));
		nd->dcons = ap_abstract0_top(pr->man_dcons,
				r->datadim + 2 * r->segmentdim + 2 * 2, 0);
		nd->key.type = 1;
		if (fst_node_tdim[0] < snd_node_tdim[0]) {
...

	{
		...
		if (n_dcons) {
			...
			n_dcons->dcons = ap_abstract0_bottom(pr->man_dcons,
					r->datadim + 2 * r->segmentdim + 2 * nr_y, 0);
		} else {
			checked_malloc(n_dcons, pattern_t, 1,
					(sizeof(pattern_t)+u_seg*sizeof(size_t)), return NULL;);
		...
		if (old_pattern_ctr)
			aux = ap_abstract0_copy(pr->man_dcons, old_pattern_ctr);
		...
		for (i = 1; i < size; i++) {
			...
			if (ap_abstract0_sat_lincons(pr->man_dcons, r->econs, &cons)) {
				...
				if (n_dcons) {
					...
					ap_abstract0_t *transf_n_dcons = ap_abstract0_copy(
							pr->man_dcons, n_dcons->dcons);

					ap_linexpr0_t *fst_expr_y = ap_linexpr0_alloc(
							AP_LINEXPR_DENSE,
							r->datadim + 2 * r->segmentdim + 4);
					ap_linexpr0_t *snd_expr_y = ap_linexpr0_alloc(
							AP_LINEXPR_DENSE,
							r->datadim + 2 * r->segmentdim + 4);
					...
					transf_n_dcons = ap_abstract0_substitute_linexpr(
							pr->man_dcons, true, transf_n_dcons, ly1,
							fst_expr_y, NULL);
					transf_n_dcons = ap_abstract0_substitute_linexpr(
							pr->man_dcons, true, transf_n_dcons, ly2,
							snd_expr_y, NULL);

					if (aux)
						aux = ap_abstract0_join(pr->man_dcons, true, aux,
								transf_n_dcons);
					else {
				...
				val_i = ap_abstract0_add_dimensions(pr->man_dcons, false,
				...
				arr.p[2].linexpr0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
						r->datadim + 2 * r->segmentdim + 4);
				arr.p[2].scalar = NULL;
				...
				arr.p[3].linexpr0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
						r->datadim + 2 * r->segmentdim + 4);
				arr.p[3].scalar = NULL;
				...
				ap_linexpr0_t *expr_y_tmp = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
						r->datadim + 2 * r->segmentdim + 4);
				ap_linexpr0_set_cst_scalar_int(expr_y_tmp, 0);

				ap_linexpr0_set_coeff_scalar_int(expr_y_tmp,
						r->datadim + 2 * r->segmentdim + 2, 1);
				ap_dim_t dn1 = r->datadim + fst_node_tdim[i];

				val_i = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
						val_i, &arr);
				ap_lincons0_array_clear(&arr);
				...
				aux = ap_abstract0_top(pr->man_dcons,
						r->datadim + 2 * r->segmentdim + 4, 0);
			}
#ifndef NDEBUG2
		...
		if (n_dcons) {
			if (n_dcons->dcons)
				n_dcons->dcons = ap_abstract0_join(pr->man_dcons, true, aux,
						n_dcons->dcons);
			else {
	...
	if (update_length) {
		...
		ap_linexpr0_t *expr2 = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
				r->datadim + 2 * r->segmentdim);
		...
		ap_linexpr0_t *expr_11y = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
				r->datadim + 2 * r->segmentdim + 2);
		...
		ap_linexpr0_t *expr_12y = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
				r->datadim + 2 * r->segmentdim + 2);
		...
		ap_linexpr0_t *expr_21y = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
				r->datadim + 2 * r->segmentdim + 4);
		...
		ap_linexpr0_t *expr_22y = ap_linexpr0_alloc(AP_LINEXPR_DENSE,
				r->datadim + 2 * r->segmentdim + 4);
		...
		for (s = r->udcons; s != NULL; s = s->hh.next) {
			if (pr->PI[s->key.type].nr_y == 2) {
				s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true,
						s->dcons, l01, expr_21y, NULL);
				s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true,
						s->dcons, l02, expr_22y, NULL);
			} else {
				s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true,
						s->dcons, l01, expr_11y, NULL);
				s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true,
						s->dcons, l02, expr_12y, NULL);
			}
		}
...
ucons_t * split_with_pattern_P21(ucons_internal_t* pr, ucons_t *r,
	...
	if (aux) {
		...
		checked_malloc(pat_y_lx, pattern_key_t, 1,
				sizeof(pattern_key_t) + (u_seg_y_lx + e_seg_y_lx)*sizeof(size_t),
				return NULL;);

		pat_y_lx->type = get_pattern_type(pr, u_seg_y_lx, e_seg_y_lx, nr_y_y_lx,
				pattern_1_lx);
		...
		if (pat_y_lx->type != 100) {
			...
			unsigned keylen_y_lx = (u_seg_y_lx + e_seg_y_lx) * sizeof(size_t)
					+ sizeof(pattern_key_t);
			for (size_t i = 0; i < e_seg; i++) {
				pat_y_lx->segments[u_seg_y_lx + i] = pattern_j->segments[u_seg
						+ i];
			}
			...
			for (size_t ii = 2; ii < 1 + e_seg_y_lx; ii++) {
				size_t jj = 1;
				while (jj != ii
						&& pat_y_lx->segments[jj] <= pat_y_lx->segments[ii])
					jj++;
				if (jj < ii) {
			...
			dcons_y_lx = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
					dcons_y_lx, &arr);
			ap_lincons0_array_clear(&arr);
			...
			tdim[0] = y_pos;
			tdim[1] = dy_pos;
			dcons_y_lx = ap_abstract0_forget_array(pr->man_dcons, true,
					dcons_y_lx, tdim, 1, true);

			ap_dimchange_t* dimchange = ap_dimchange_alloc(2, 0);
			dimchange->dim[0] = y_pos;
			dimchange->dim[1] = dy_pos;
			dcons_y_lx = ap_abstract0_remove_dimensions(pr->man_dcons, true,
					dcons_y_lx, dimchange);
			ap_dimchange_free(dimchange);

			pattern_t * pattern_y_lx;
			HASH_FIND(hh, r->udcons, pat_y_lx, keylen, pattern_y_lx);
			if (pattern_y_lx) {
				pattern_y_lx->dcons = ap_abstract0_meet(pr->man_dcons, true,
						pattern_y_lx->dcons, dcons_y_lx);
			} else {
				checked_malloc(pattern_y_lx, pattern_t, 1,
						sizeof(pattern_t)+(u_seg_y_lx+e_seg_y_lx)*sizeof(size_t),
						return NULL;);
				memset(pattern_y_lx, 0,
						sizeof(pattern_t)
								+ (u_seg_y_lx + e_seg_y_lx) * sizeof(size_t));
				...
				pattern_y_lx->dcons = ap_abstract0_copy(pr->man_dcons,
						dcons_y_lx);
		...
		checked_malloc(pat_2_1_lx, pattern_key_t, 1,
				sizeof(pattern_key_t) + (u_seg_2_lx + e_seg_2_lx)*sizeof(size_t),
				return NULL;);
		 ...
		 * */
		pat_2_1_lx->type = get_pattern_type(pr, u_seg_2_lx, e_seg_2_lx,
				nr_y_2_lx, pattern_2_1_lx);

		if (pat_2_1_lx->type != 100) {
			ap_abstract0_t* dcons_y1y2_lx;
			dcons_y1y2_lx = ap_abstract0_copy(pr->man_dcons, aux->dcons);

			unsigned keylen_2_lx = (u_seg_2_lx + e_seg_2_lx) * sizeof(size_t)
					+ sizeof(pattern_key_t);
			for (size_t i = 0; i < e_seg; i++) {
				pat_2_1_lx->segments[u_seg_2_lx + i] = pattern_j->segments[u_seg
						+ i];
			}

			pat_2_1_lx->segments[u_seg_2_lx + e_seg_2_lx - 1] = n1;

			for (size_t ii = u_seg_2_lx + 1; ii < u_seg_2_lx + e_seg_2_lx;
					ii++) {
				size_t jj = u_seg_2_lx;
				while (jj != ii
						&& pat_2_1_lx->segments[jj] <= pat_2_1_lx->segments[ii])
					jj++;
				if (jj < ii) {
			...
			dcons_y1y2_lx = ap_abstract0_substitute_linexpr(pr->man_dcons, true,
					dcons_y1y2_lx, y_pos, expr, NULL);
			ap_linexpr0_free(expr);

			if (perm_u_seg) {
				ap_dimperm_t * perm = ap_dimperm_alloc(
						r->datadim + 2 * r->segmentdim + 2 * nr_y);
				...
				dcons_y1y2_lx = ap_abstract0_permute_dimensions(pr->man_dcons,
						true, dcons_y1y2_lx, perm);
			...
			if (pattern_2_lx) {
				pattern_2_lx->dcons = ap_abstract0_meet(pr->man_dcons, true,
						pattern_2_lx->dcons, dcons_y1y2_lx);
				/* !y_pos >= 1  */
				...
				pattern_2_lx->dcons = ap_abstract0_meet_lincons_array(
						pr->man_dcons, true, pattern_2_lx->dcons, &arr);
				...
				checked_malloc(pattern_2_lx, pattern_t, 1,
						sizeof(pattern_t)+(u_seg_2_lx+e_seg_2_lx)*sizeof(size_t),
						return NULL;);
				memset(pattern_2_lx, 0,
						sizeof(pattern_t)
								+ (u_seg_2_lx + e_seg_2_lx) * sizeof(size_t));
				...
				pattern_2_lx->dcons = ap_abstract0_copy(pr->man_dcons,
						dcons_y1y2_lx);
				/* !y_pos >= 1  */
...
bool test_equal_length(ap_manager_t * man, ap_abstract0_t * dcons,
		size_t intdim, size_t realdim, ap_dim_t n, ap_dim_t m) {
...
bool test_l_leq_v(ap_manager_t * man, ap_abstract0_t * dcons, size_t intdim,
		size_t realdim, ap_dim_t n, int v) {
...
bool test_g_leq_v(ap_manager_t * man, ap_abstract0_t * dcons, size_t intdim,
		size_t realdim, ap_dim_t n, int v) {
...
ap_dim_t * equal_lengths(ap_manager_t * man, ap_abstract0_t * dcons,
		size_t intdim, size_t realdim, ap_dim_t n, ap_dim_t *tdim,
		size_t size_tdim, size_t* size) {

	ap_dim_t *len = NULL;
	size_t size_ps = realdim - 1 - size_tdim;
	ap_dim_t *ps = (ap_dim_t*) malloc(
			(realdim - 1 - size_tdim) * sizeof(ap_dim_t));
...
ap_lincons0_t cons_dy0_dy1(size_t intdim, size_t realdim, ap_dim_t dy0,
		ap_dim_t dy1) {
