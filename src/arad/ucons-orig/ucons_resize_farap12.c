 ...
 */
ucons_t *
ucons_forget_array(ap_manager_t * man, bool destructive, ucons_t * a,
		ap_dim_t * tdim, size_t size, bool project) {
	ucons_internal_t *pr = ucons_init_from_manager(man, AP_FUNID_FORGET_ARRAY,
			0);
...
/* TODO: priority 0 */
ucons_t *
ucons_add_dimensions(ap_manager_t * man, bool destructive, ucons_t * a,
		ap_dimchange_t * dimchange, bool project) {
	ucons_internal_t *pr = ucons_init_from_manager(man, AP_FUNID_ADD_DIMENSIONS,
			0);
	...
	ucons_t *r = ucons_alloc_internal(pr, a->datadim + addi,
			a->segmentdim + nbseg);
	...
	r->econs = ap_abstract0_add_dimensions(pr->man_dcons, false, a->econs,
			&dimadd, project);
	...
	for (s = a->udcons; s != NULL; s = s->hh.next) {
		...
		if (rt) {
			rt->dcons = ap_abstract0_add_dimensions(pr->man_dcons, destructive,
					s->dcons, &dimadd, project);
		} else {

			checked_malloc(rt, pattern_t, 1,
...
ucons_t* add_pattern_n2p(ucons_internal_t *pr, ucons_t *r, pattern_key_t * key) {
	...
	for (i = 0; i < (u_seg + e_seg); i++) {
		if (!(key->segments[i] < r->segmentdim))
			return NULL;
		dim = key->segments[i];

		size = r->n2p[dim].size;
		checked_realloc(r->n2p[dim].p, pattern_key_t*, (r->n2p[dim].size+1),
				sizeof(pattern_key_t *), return NULL;);
		checked_malloc(r->n2p[dim].p[size], pattern_key_t, 1,
...
ucons_t* remove_pattern_n2p(ucons_internal_t *pr, ucons_t *r,
		pattern_key_t * key) {
#ifndef NDEBUG1
	fprintf(stdout,"\n remove_pattern_n2p \n");
	//ucons_fprint(stdout,pr->man,r,NULL);
	...
	for (i = 0; i < (u_seg + e_seg); i++) {
		...
		if (size > 0) {
			for (size_t curr = 0; curr < size; curr++) {
				...
				if (type_ctr == type && u_seg_ctr == u_seg
						&& e_seg_ctr == e_seg) {
					if (size == 1) {
						free(r->n2p[dim].p);
						r->n2p[dim].p = NULL;
						...
						checked_realloc(r->n2p[dim].p, pattern_key_t*,
								(r->n2p[dim].size), sizeof(pattern_key_t *),
								return NULL;);
						curr = size;
...
/* TODO: priority 0 */
ucons_t *
ucons_remove_dimensions(ap_manager_t * man, bool destructive, ucons_t * a,
		ap_dimchange_t * dimchange) {
	ucons_internal_t *pr = ucons_init_from_manager(man,
			AP_FUNID_REMOVE_DIMENSIONS, 0);
	...
	ucons_t *r = ucons_alloc_internal(pr, a->datadim - dimchange->intdim,
			a->segmentdim - dimchange->realdim);
	...
	for (i = 0; i < dimchange->realdim; i++) {
		dimrm.dim[dimchange->intdim + 2 * i + 0] =
				dimchange->dim[dimchange->intdim + i];
		dimrm.dim[dimchange->intdim + 2 * i + 1] = a->segmentdim
				+ dimchange->dim[dimchange->intdim + i];
	}
	// dimrm shall be sorted!
	shape_dimchange_sort(&dimrm);
	r->econs = ap_abstract0_remove_dimensions(pr->man_dcons, destructive,
			a->econs, &dimrm);
		...
		if (new_segments_size == (u_seg + e_seg)) {
			...
			for (i = 0; i < (u_seg + e_seg); i++) {
				if (i < u_seg)
					...
					rt->key.segments[i] = new_e_seg[i - u_seg];
				j = 0;
				while (j < dimchange->realdim) {
					if ((i < u_seg
							&& s->key.segments[i]
									>= (dimchange->dim[j] - a->datadim))
							|| (i >= u_seg
									&& new_e_seg[i - u_seg]
											>= (dimchange->dim[j] - a->datadim))) {
						rt->key.segments[i]--;
			...
			if (r_pattern) {
				r_pattern->dcons = ap_abstract0_remove_dimensions(pr->man_dcons,
						destructive, s->dcons, &dimrm);
			} else {

				checked_malloc(r_pattern, pattern_t, 1,
						(sizeof(pattern_t)+(u_seg+e_seg)*sizeof(size_t)),
						return NULL;);
				memset(r_pattern, 0,
						(sizeof(pattern_t) + (u_seg + e_seg) * sizeof(size_t)));
...
/* TODO: priority 0 */
ucons_t *
ucons_permute_dimensions(ap_manager_t * man, bool destructive, ucons_t * a,
		ap_dimperm_t * perm) {
	ucons_internal_t *pr = ucons_init_from_manager(man, AP_FUNID_ADD_DIMENSIONS,
			0);
	if (!a)
...
//	printf("\n");
	...
	new_node_perm.dim = (ap_dim_t *) malloc(
			(perm->size - a->datadim) * sizeof(ap_dim_t));
	...
	for (i = 0, j = 0, rmsize = 0; i < perm->size; i++)
		if (i > a->datadim && perm->dim[i] == a->datadim) {
			...
			if (j >= a->datadim)
				new_node_perm.dim[j - a->datadim] = perm->dim[i] - a->datadim;
			j++;
		}
	if (rmsize > 0) {
		dimrm.intdim = 0;
		dimrm.realdim = rmsize;
		dimrm.dim = (ap_dim_t *) realloc(dimrm.dim, rmsize * sizeof(ap_dim_t));
		...
		newperm.dim = (ap_dim_t *) realloc(newperm.dim,
				newperm.size * sizeof(ap_dim_t));

		new_node_perm.size = new_node_perm.size - rmsize;
		new_node_perm.dim = (ap_dim_t *) realloc(new_node_perm.dim,
				new_node_perm.size * sizeof(ap_dim_t));
	...
	for (i = 0; i < newsegmsize; i++) {
		...
		consperm.dim[a->datadim + newsegmsize + i] = newsegmsize
	...
	for (s = r->udcons; s != NULL; s = s->hh.next) {
		...
		ra->dcons =
				(s->dcons) ? ap_abstract0_copy(pr->man_dcons, s->dcons) : NULL;
		...
		if (sort == false) {
			for (i = 0; i < u_seg; i++) {
				consperm.dim[a->datadim + 2 * newsegmsize + 2 * i] = a->datadim
						+ 2 * newsegmsize + (2 * i) + 1;
				consperm.dim[a->datadim + 2 * newsegmsize + 2 * i + 1] =
						a->datadim + 2 * newsegmsize + (2 * i);
			}

			if (pr->PI[ra->key.type].kind == pattern_2_1_lx)
				ra->key.type = get_pattern_type(pr, u_seg, e_seg, nr_y,
						pattern_2_1_mlx);
		...
		//#endif
		ra->dcons = ap_abstract0_permute_dimensions(pr->man_dcons, true,
				ra->dcons, &consperm);
	...
	for (size_t i = 0; i < r->segmentdim; i++) {
		...
		checked_malloc(r->n2p[new_i].p, pattern_key_t*, size_new_i,
				sizeof(pattern_key_t), return NULL;);
		...
		for (size_t j = 0; j < size_new_i; j++) {
			...
			for (size_t ii = 1; ii < u_seg; ii++) {
				size_t jj = 0;
				while (jj != ii
						&& r->n2p[new_i].p[j]->segments[jj]
								<= r->n2p[new_i].p[j]->segments[ii])
					jj++;
				if (jj < ii) {
					...
					for (tmp = ii; tmp > jj; tmp--) {
						kk = tmp - 1;
						r->n2p[new_i].p[j]->segments[kk + 1] =
								r->n2p[new_i].p[j]->segments[kk];
					}
					//							for (size_t  kk = ii - 1; kk >= jj; kk--)
...
ucons_singleton(ucons_internal_t * pr, bool destructive, ucons_t * a,
	...
	r->econs = ap_abstract0_meet_lincons_array(pr->man_dcons, false, a->econs,
			&arr);
	...
	for (s = r->udcons; s != NULL;) {
		...
		found = false;
		for (i = 0; i < (u_seg); i++)
			if (s->key.segments[i] == dim)
				found = true;

		total_dim = a->datadim + 2 * a->segmentdim
				+ 2 * pr->PI[s->key.type].nr_y;

		arr = ap_lincons0_array_make(1);
		// l[dim]-1 == 0
		arr.p[0].constyp = AP_CONS_EQ;
		arr.p[0].linexpr0 = ap_linexpr0_alloc(AP_LINEXPR_DENSE, total_dim);
		arr.p[0].scalar = NULL;
		ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0,
				a->datadim + a->segmentdim + dim, 1);
		ap_linexpr0_set_cst_scalar_int(arr.p[0].linexpr0, -1);
		...
		if (found) {
			...
			s->dcons = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
					s->dcons, &arr);
...
bool test_pattern_sat(ucons_internal_t * pr, ucons_t *r, pattern_key_t *p,
		int s) {
...
ucons_split(ucons_internal_t * pr, bool destructive, ucons_t * a, ap_dim_t dim) {
	...
	// meet l[n1]-2 >= 0
	r->econs = ap_abstract0_meet_lincons_array(pr->man_dcons, true, r->econs,
			&arrr);
	...
	for (sp = r->udcons; sp != NULL; sp = sp->hh.next) {
		...
		if (!ap_abstract0_is_bottom(pr->man_dcons, sp->dcons)) {
			sp->dcons = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
					sp->dcons, &arrr);
		}
		ap_lincons0_array_clear(&arrr);
	}
				...
				if (pattern_sat) {
					...
					for (i = 0; i < u_seg && !found_pos_y; i++) {
						if (pattern_j->segments[i] == n1) {
							...
							pos_y += pr->PI[pattern_j->type].uvar[i].size;
					}

					if (nr_y > 1) {
						/* if the pattern is different from \forall y */
						checked_malloc(tdim, ap_dim_t, 2*(nr_y-1),
						...
						for (i = 0; i < 2 * nr_y; i++) {
							if (i != (pos_y - 1) && i != nr_y + (pos_y - 1)) {
								tdim[contor] = r->datadim + 2 * r->segmentdim
										+ i;
								contor += 1;
							}
						}
						...
						n2_aux = ap_abstract0_remove_dimensions(pr->man_dcons,
								true, n2_aux, dimchange);
					...
					arr.p[0].scalar = NULL;
					ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, pos_y,
							1);
					ap_linexpr0_set_cst_scalar_int(arr.p[0].linexpr0, -1);
					...
					n2_aux = ap_abstract0_meet_lincons_array(pr->man_dcons,
							false, n2_aux, &arr);
					ap_lincons0_array_clear(&arr);
					...
					tdim[0] = pos_y;
					tdim[1] = pos_y + 1;
					n2_aux = ap_abstract0_forget_array(pr->man_dcons, false,
							n2_aux, tdim, 1, true);
					//  remove dimension pos_y, pos_y+1
					dimchange = ap_dimchange_alloc(2, 0);
					dimchange->dim[0] = pos_y;
					dimchange->dim[1] = pos_y + 1;
					n2_aux = ap_abstract0_remove_dimensions(pr->man_dcons, true,
							n2_aux, dimchange);
					ap_dimchange_free(dimchange);
					...
					if (econs_aux == NULL)
						econs_aux = ap_abstract0_copy(pr->man_dcons, n2_aux);
					else
						econs_aux = ap_abstract0_meet(pr->man_dcons, true,
								econs_aux, n2_aux);
	...
	for (j = 0; j < n1_pat_set.size; j++) {
		...
		if (pattern_j->type == 1
				|| (pr->PI[pattern_j->type].kind == pattern_2_1_lx
						&& pattern_j->segments[1] == n1)
				|| (pr->PI[pattern_j->type].kind == pattern_2_1_mlx
						&& pattern_j->segments[0] == n1)) {
			/*
	...
	if (econs_aux) {
		...
		for (s = r->udcons; s != NULL; s = s->hh.next) {
			...
			udcons_aux = ap_abstract0_add_dimensions(pr->man_dcons, false,
					econs_aux, &dimadd, false);
			//free(dimadd.dim);
			ap_dimchange_clear(&dimadd);
			s->dcons = ap_abstract0_meet(pr->man_dcons, true, udcons_aux,
					s->dcons);
			udcons_aux = NULL;
		}
	}
	...
	ap_linexpr0_set_cst_scalar_int(expr, 1);
	r->econs = ap_abstract0_substitute_linexpr(pr->man_dcons, true, r->econs,
			ln1, expr, NULL);

	// l[n1] := 1;
	ap_linexpr0_set_coeff_scalar_int(expr, ln2, 0);

	r->econs = ap_abstract0_assign_linexpr(pr->man_dcons, true, r->econs, ln1,
			expr, NULL);
	...
	for (s = r->udcons; s != NULL; s = s->hh.next) {
		...
		ap_linexpr0_set_coeff_scalar_int(expr, ln2, 1);
		ap_linexpr0_set_cst_scalar_int(expr, 1);
		s->dcons = ap_abstract0_substitute_linexpr(pr->man_dcons, true,
				s->dcons, ln1, expr, NULL);
		// l[n1] := 1;
		ap_linexpr0_set_coeff_scalar_int(expr, ln2, 0);
		s->dcons = ap_abstract0_assign_linexpr(pr->man_dcons, true, s->dcons,
				ln1, expr, NULL);
...
ucons_t *
ucons_expand(ap_manager_t * man, bool destructive, ucons_t * a, ap_dim_t dim,
		size_t n) {
...
ucons_t *
ucons_fold(ap_manager_t * man, bool destructive, ucons_t * a, ap_dim_t * tdim,
		size_t size) {
		...
		if (first < pr->segm_anon) {
			...
			if (first == 1) {
				if (pr->active_patterns[1]) {
					...
					r = fold_with_closoure_of_P21(pr, a, fold_dim,
							size_fold_dim, 0, false);
					...
					r = fold_with_closoure_of_P21(pr, a, fold_dim,
							size_fold_dim, 0, true);
				}
			} else {
				/* segmanon = 2 therefore complicated fold with/without
				 *  closure and (posibelly) with the other patterns */
#ifndef NDEBUG1
				fprintf(stdout,"\t size_fold_dim=%zu \t fold_dim: \n",size_fold_dim);
				for(size_t o = 0;o<size_fold_dim;o++)
				fprintf(stdout,"\t %zu ",fold_dim[o]);
...
ucons_t * merge_succesors_2(ucons_internal_t *pr, ucons_t * r, ap_dim_t * gdim,
	...
	if (n_dcons) {
		...
		for (i = 1; i < size_gdim; i++) {
			...
			udcons = ap_abstract0_add_dimensions(pr->man_dcons, false, r->econs,
					dimchange, false);
			...
			udcons = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
					udcons, &arr);
	...
	if (!n2_dcons) {
		checked_malloc(n2_dcons, pattern_t, 1,
				sizeof(pattern_t)+u_seg2*sizeof(size_t), return NULL;);
	...
	if (n2_dcons) {
		...
		for (i = 1; i < size_gdim; i++) {
			for (j = i; j < size_gdim; j++) {
				...
				udcons2 = ap_abstract0_add_dimensions(pr->man_dcons, false,
				...
				udcons2 = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
						udcons2, &arr);
				...
				if (n2_dcons->dcons == NULL) {
					...
					n2_dcons->dcons = ap_abstract0_join(pr->man_dcons, true,
							n2_dcons->dcons, udcons2);
		...
		if (n2_dcons->dcons == NULL)
			n2_dcons->dcons = ap_abstract0_bottom(pr->man_dcons,
	...
	if (update_lenght) {
		...
		r->econs = ap_abstract0_substitute_linexpr(pr->man_dcons, true,
				r->econs, ln, expr, NULL);
		// l(gdim[0]) := size_gdim
	...
	for (i = 1; i < size_cdim; i++) {
		...
		if (n_dcons) {
			if (n_dcons->dcons) {
				...
				fy_dcons = ap_abstract0_substitute_linexpr(pr->man_dcons, true,
						fy_dcons, ly, expr_y, NULL);
				if (aux_dcons)
					aux_dcons = ap_abstract0_join(pr->man_dcons, true,
							aux_dcons, fy_dcons);
				else
					aux_dcons = ap_abstract0_copy(pr->man_dcons, fy_dcons);
		...
		ni_dcons = ap_abstract0_add_dimensions(pr->man_dcons, false, ni_dcons,
				&dimchange, false);
		free(dimchange.dim);
		...
		ni_dcons = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
				ni_dcons, &arr);
		ap_lincons0_array_clear(&arr);

#ifndef NDEBUG1
		printf("\n CONCAT_P11 aux_dcons ");
		if(ni_dcons) ap_abstract0_fprint(stdout, pr->man_dcons, ni_dcons, NULL);
		printf("\n");
#endif
		if (aux_dcons)
			aux_dcons = ap_abstract0_join(pr->man_dcons, true, aux_dcons,
					ni_dcons);
		else {
...
ap_abstract0_t *concat_P12(ucons_internal_t * pr, ucons_t * r, ap_dim_t * cdim,
	...
	for (i = 1; i < size_cdim; i++) {
		...
		//				fy2_dcons = ap_abstract0_bottom(pr->man_dcons);
		//			else return r;
		//		}
		if (!found2_dcons) {
			if (!test_singleton(pr->man_dcons, r->econs, r->datadim,
		...
		if (found2_dcons) {
			if (found2_dcons->dcons) {
				...
				fy2_dcons = ap_abstract0_copy(pr->man_dcons,
						found2_dcons->dcons);
				fy2_dcons = ap_abstract0_substitute_linexpr(pr->man_dcons,
						false, fy2_dcons, ly1, expr_2y, NULL);

				ap_linexpr0_set_coeff_scalar_int(expr_2y, ly1, 0);
				ap_linexpr0_set_coeff_scalar_int(expr_2y, ly2, 1);

				fy2_dcons = ap_abstract0_substitute_linexpr(pr->man_dcons, true,
						fy2_dcons, ly2, expr_2y, NULL);

				ap_linexpr0_free(expr_2y);

				if (aux_2y_dcons)
					aux_2y_dcons = ap_abstract0_join(pr->man_dcons, true,
							aux_2y_dcons, fy2_dcons);
				else {
		...
		for (wr = i; wr < size_cdim; wr++) {
			...
			if (found_right_dcons) {
				if (found_right_dcons->dcons) {
					...
					border = ap_abstract0_copy(pr->man_dcons,
							found_right_dcons->dcons);
					...
					border = ap_abstract0_substitute_linexpr(pr->man_dcons,
							true, border, ly2, expr_b, NULL);
					...
					dy1_econs = ap_abstract0_add_dimensions(pr->man_dcons,
							false, r->econs, &dimchange, false);
					free(dimchange.dim);
					...
					for (j = 0; j < i; j++) {
						li = r->datadim + r->segmentdim + cdim[j];
						ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, li,
								1);
					}
					ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, ly1,
							-1);
					//ap_linexpr0_set_cst_scalar_int (arr.p[0].linexpr0, 0);
					...
					ap_linexpr0_set_coeff_scalar_int(arr.p[1].linexpr0,
							r->datadim + cdim[i], 1);
					ap_linexpr0_set_coeff_scalar_int(arr.p[1].linexpr0, dy1,
							-1);
					//ap_linexpr0_set_cst_scalar_int (arr.p[1].linexpr0, 0);
					/* DO NOT DELETE old in*/
					dy1_econs = ap_abstract0_meet_lincons_array(pr->man_dcons,
							true, dy1_econs, &arr);
					border = ap_abstract0_meet(pr->man_dcons, true, dy1_econs,
							border);
		...
		for (wl = i - 1; wl >= 0; wl--) {
			...
			if (found_left_dcons) {
				if (found_left_dcons->dcons) {
					...
					dy2_econs = ap_abstract0_add_dimensions(pr->man_dcons,
							false, r->econs, &dimchange, false);
					free(dimchange.dim);
					ap_lincons0_array_t arr = ap_lincons0_array_make(2);
					...
					ap_linexpr0_set_coeff_scalar_int(arr.p[0].linexpr0, ly2,
							-1);
					//ap_linexpr0_set_cst_scalar_int (arr.p[0].linexpr0, 0);
					...
					ap_linexpr0_set_coeff_scalar_int(arr.p[1].linexpr0,
							r->datadim + cdim[i], 1);
					ap_linexpr0_set_coeff_scalar_int(arr.p[1].linexpr0, dy2,
							-1);
					//ap_linexpr0_set_cst_scalar_int (arr.p[1].linexpr0, 0);
					...
					dy2_econs = ap_abstract0_meet_lincons_array(pr->man_dcons,
							true, dy2_econs, &arr);
					border = ap_abstract0_meet(pr->man_dcons, true, dy2_econs,
							border);
					/*Old out*/
					//					border = ap_abstract0_meet_lincons_array (pr->man_dcons, true, border, &arr);
					ap_lincons0_array_clear(&arr);
		...
		if (i + 1 < size_cdim) {
			...
			dy12_econs = ap_abstract0_meet_lincons_array(pr->man_dcons, true,
					dy12_econs, &arr);
			ap_lincons0_array_clear(&arr);
			...
			for (j = i + 1; j < size_cdim; j++) {
				...
				dy12_ucons = ap_abstract0_meet_lincons_array(pr->man_dcons,
						true, dy12_ucons, &arr);
				ap_lincons0_array_clear(&arr);
...
ucons_t* concat_nodes_2(ucons_internal_t *pr, ucons_t * r, ap_dim_t* cdim,
		...
		if (n_dcons->dcons) {
			...
			if (!test_singleton(pr->man_dcons, r->econs, r->datadim,
				...
				if (aux_dcons == NULL) {
					...
					n_dcons->dcons = ap_abstract0_copy(pr->man_dcons,
							aux_dcons);
			}
		}
	} else {
	...
	if (found2_dcons) {
		if (found2_dcons->dcons) {
			if (aux_2y_dcons)
				found2_dcons->dcons = ap_abstract0_join(pr->man_dcons, false,
						aux_2y_dcons, found2_dcons->dcons);
		} else {
			if (!test_singleton(pr->man_dcons, r->econs, r->datadim,
					r->segmentdim, cdim[0])) {
				...
				if (aux_2y_dcons == NULL) {
					...
					found2_dcons->dcons = ap_abstract0_copy(pr->man_dcons,
							aux_2y_dcons);
			}
		}
	} else {
		if (!test_singleton(pr->man_dcons, r->econs, r->datadim, r->segmentdim,
			...
			checked_malloc(found2_dcons, pattern_t, 1,
					sizeof(pattern_t)+u_seg*sizeof(size_t), return NULL;);
			memset(found2_dcons, 0, sizeof(pattern_t) + u_seg * sizeof(size_t));
			n_dcons->key.type = look2->type;
			for (size_t i = 0; i < (u_seg); i++)
	...
	ap_abstract0_free(pr->man_dcons, aux_2y_dcons);
	...
	for (i = 1; i < size_cdim; i++) {
		...
		if (n_dcons) {
			...
			if (n_dcons->dcons) {
				...
				ly = r->datadim + 2 * r->segmentdim;
				ap_linexpr0_set_coeff_scalar_int(expr_y, ly, 1);
				n_dcons->dcons = ap_abstract0_substitute_linexpr(pr->man_dcons,
						true, n_dcons->dcons, ly, expr_y, NULL);
				if (aux_dcons)
					aux_dcons = ap_abstract0_join(pr->man_dcons, true,
							aux_dcons, n_dcons->dcons);
				else
					aux_dcons = ap_abstract0_copy(pr->man_dcons,
							n_dcons->dcons);
		...
		ni_dcons = ap_abstract0_add_dimensions(pr->man_dcons, false, r->econs,
				&dimchange, false);
		free(dimchange.dim);
