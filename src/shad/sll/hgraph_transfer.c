...
void
hgraph_node_anon_array_clear (ap_dim_array2_t * anon)
{
  assert (NULL != anon);
  if (anon && anon->size)
    {
                            ...
                            ap_dimperm_t * perm, ap_dim_array2_t * anon)
{
  ...
  hgraph_t *r = NULL;           /* TO TEST */
  if (conservative == true)
    r = hgraph_copy_mem (pr, a);
  else
    r = a;
  r = hgraph_assign_x_null (pr, r, vx, perm, anon);     /* changed 2nd param to r */
 ...
 */
hgraph_t *
hgraph_substitute_x_null (hgraph_internal_t * pr, hgraph_t * a, size_t vx,
 ...
 */
hgraph_t *
hgraph_substitute_x_next_null (hgraph_internal_t * pr, hgraph_t * a,
                               size_t vx, size_t vy, ap_dimperm_t * perm,
                               ap_dim_array2_t * anon)
{
  arg_assert (perm->size == a->size, return NULL;);
...
{
  if (op->lhs < a->datadim || op->op == PA_ASSIGN_INT)
  ...
  if (op->offx == OFFSET_NONE)
  ...
  else if (op->offx == ((int) (a->datadim + OFFSET_NEXT)))
    {
      ...
      if (IS_NULLDIM (op->info.ptr.y) || ny == 0)
        {                       // x->next = null
          if (conservative)
            {
              ap_dimperm_init (&perm, a->size);
  ...
  if (hgraph_is_bottom (man, a))
  ...
  else
    {
      ...
      if (!destructive)
        ...
        b = a;
      op = shape_passign_of_linexpr_array (&d, &expr, 1, dim.intdim,
                                           dim.realdim);
#ifndef NDEBUG
                           ...
                           ap_texpr0_t ** texpr, size_t size, hgraph_t * dest)
{
  if (hgraph_is_bottom (man, a))
  ...
  else
    {
      ...
      hgraph_array_t *r;
      hgraph_internal_t *pr =
        sh_init_from_manager (man, AP_FUNID_ASSIGN_TEXPR_ARRAY, 0);
      ap_dimension_t dim = hgraph_dimension (man, a);
      if (!destructive)
        b = hgraph_copy_mem (pr, a);
      else
