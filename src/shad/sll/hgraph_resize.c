...
hgraph_dup_dimensions (hgraph_internal_t * pr, hgraph_t * a,
                       ap_dimchange_t * dimchange, ap_dimperm_t * perm,
                       ap_dimperm_t * eqrel)
{
  ...
  arg_assert (dimchange &&
              ...
              ((dimchange->realdim > 0) ?
               ((dimchange->dim[dimchange->intdim] ==
                 (a->datadim + a->ptrdim))
                || (dimchange->dim[dimchange->intdim] == a->datadim)) : 1),
              return NULL;
    );

  /* Requirement on perm */
  arg_assert (perm && (perm->size == (2 * a->size - 1)), return NULL;
    );

  /* Requirement on eqrel */
  arg_assert (eqrel && (eqrel->size == a->size), return NULL;
    );
  ...
  for (v = 0; v < a->ptrdim; v++)
    {
      ...
      VAR2NODE (r, dupv) = (VAR2NODE (a, v) == NODE_NULL) ? NODE_NULL : (VAR2NODE (a, v) + a->size - 1);        // -1 because NODE_NULL not copied
    }
                       ...
                       ap_dimchange_t * dimchange, bool project)
{
  ...
  if (project != project)       /* remove gcc warning */
    return NULL;
  ...
  if (dimchange->realdim == 0)
  ...
  else
    {
      ...
      checked_malloc (newvars, size_t, sizeof (size_t), a->ptrdim,
                      return NULL;);
      ...
      for (i = 0, j = 0, k = 0; j < dimchange->realdim || i < nptrdim; i++)
        if (j < dimchange->realdim
            && ((dimchange->dim[j + dimchange->intdim] - a->datadim) ==
                (i - k)))
          {
...
hgraph_remove_dimensions_internal (hgraph_internal_t * pr, bool destructive,
                                   hgraph_t * a, ap_dimchange_t * dimchange,
                                   ap_dimperm_t * perm)
{
  if (!a)
    return a;
  ...
  if (dimchange->realdim > 0)
    {
      ...
      size_t *newvars;          // maps old vars in new vars
      checked_malloc (newvars, size_t, sizeof (size_t), rr->ptrdim,
                      return NULL;);
      /* start copying the ptrdim part of info */
...
hgraph_permute_dimensions_internal (hgraph_internal_t * pr, bool destructive,
                                    hgraph_t * a, ap_dimperm_t * inperm,
...
#ifndef NDEBUG1
  fprintf (stdout,
           "!!!!hgraph_permute_dimensions_internal: after permuting dimensions r=(");
  hgraph_fdump (stdout, pr->man, r);
  fprintf (stdout, ")\n");
  fflush (stdout);
#endif
