...
/**
 * @brief Not implemented because we do forward analysis.
 */
ushape_t *
...
ushape_dup_dimensions (ushape_internal_t * pr,
                       ushape_t * a, ap_dimchange_t * dimchange)
{
  ...
  arg_assert (dimchange &&
              ...
              ((dimchange->realdim > 0) ?
               ((dimchange->dim[dimchange->intdim] == a->datadim) ||
                (dimchange->dim[dimchange->intdim] ==
                 (a->datadim + a->ptrdim))) : 1), return NULL;
    );
  ...
  fflush (stdout);
#endif
  ap_lincons0_array_t eqcons =
    ap_lincons0_array_make (a->datadim + a->h->size - 1);
  ...
  for (i = 0; i < a->datadim; i++)
    {
      ...
      ap_linexpr0_t *lexpr =
        ap_linexpr0_alloc (AP_LINEXPR_DENSE, b->datadim + b->h->size);
...
#ifndef NDEBUG2
      fprintf (stdout, "\n\t- (%d = %d) builds ", b->datadim + i,
               b->datadim + eqrel.dim[i]);
      ap_linexpr0_fprint (stdout, lexpr, NULL);
      fprintf (stdout, "\n");
      fflush (stdout);
#endif
      eqcons.p[a->datadim + i - 1] =
  ...
  for (i = 0; i < pr->size_scons; i++)
    r->scons[i] =
      ap_abstract0_meet_lincons_array (pr->man_scons[i], false, b->scons[i],
                                       &eqcons);
                       ...
                       ap_dimchange_t * dimchange, bool project)
{
  ...
  if (project)
  ...
  else
    {
      ...
      hgraph_t *h =
        hgraph_add_dimensions (man, false, a->h, dimchange, false);
      // nodes are not permuted because all new dimensions are set to NODE_NULL

      b =
        ushape_alloc_internal (pr, a->datadim + dimchange->intdim,
                               a->ptrdim + dimchange->realdim);
      b->h = hgraph_copy_internal (pr, h);
      hgraph_free_internal (pr, h);
      b->closed = NULL;
          ...
          if (a->scons)
            {
              ...
              for (i = 0; i < pr->size_scons; i++)
                if (a->scons[i])
                  b->scons[i] =
                    ap_abstract0_copy (pr->man_scons[i], a->scons[i]);
            }
        }
    }
  if (destructive)
  ...
  for (i = 0; i < pr->size_scons; i++)
    {
      ...
      fprintf (stdout, ")\n");
#endif
      b->scons[i] = ap_abstract0_add_dimensions (pr->man_scons[i], false, a->scons[i], change, false);  // &dimchange, false);
 ...
 * Expanding a dimension is done by add_dimension with project.
 */
ushape_t *
ushape_expand (ap_manager_t * man,
               bool destructive, ushape_t * a, ap_dim_t dim, size_t n)
{
  if ((destructive != destructive) || (dim != dim) || (n != n))
    return NULL;                /* to remove warning on unused parameter */
                      ...
                      ushape_t * a, ushape_t * b, ap_dim_array2_t * anon)
{
  ...
  ap_dimperm_clear (&perm);
  // free all anon
  hgraph_node_anon_array_clear (&fanon);
  hgraph_node_anon_array_clear (anon);
}
