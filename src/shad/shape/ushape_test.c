...
test_misc (void)
{
  ...
  if (ushape_check (pr, bot) != '.')
    printf ("ushape_bottom failed #0\n");
  if (ushape_check (pr, top) != '.')
    printf ("ushape_top failed #0\n");
  if (d1.intdim != intdim || d1.realdim != realdim)
    printf ("ushape_dimension failed #1\n");
  if (d2.intdim != intdim || d2.realdim != realdim)
    printf ("ushape_dimension failed #2\n");
  if (!ushape_is_bottom (ms, bot))
    printf ("ushape_is_bottom failed #3\n");
  if (ushape_is_bottom (ms, top))
    printf ("ushape_is_bottom failed #4\n");
  if (ushape_is_top (ms, bot))
    printf ("ushape_is_top failed #5\n");
  if (!ushape_is_top (ms, top))
    printf ("ushape_is_top failed #6\n");
  if (!ushape_is_leq (ms, bot, top))
    printf ("ushape_is_leq failed #7\n");
  if (ushape_is_leq (ms, top, bot))
    printf ("ushape_is_leq failed #8\n");
  if (!ushape_is_eq (ms, bot, bot))
    printf ("ushape_is_eq failed #9\n");
...
test_lincons_conversion (exprmode_t mode)
{
  ...
  LOOP
  {
    ...
    oo = ushape_meet_lincons_array (ms, true, o, &t);
    FLAG (ms);
    RESULT (ushape_check (pr, o));
...
test_add_pcons_once (exprmode_t mode, ushape_t * o, pcons0_array_t * array)
{
  ...
  o2 = ushape_meet_pcons_array (pr, true, o1, array);
  FLAG (ms);
  ushape_check (pr, o);
  if (o2)
    {
...
test_assign (int subst, exprmode_t mode)
{
  ...
  LOOP
  {
    ...
    if (lrand48 () % 10 >= 5)
      ushape_close (pr, o);
    o1 =
      subst ? ushape_substitute_linexpr_array (ms, false, o, &d, &l, 1,
                                               NULL) :
...
test_assign_texpr (int subst, exprmode_t mode)
{
  ...
  LOOP
  {
    ...
    if (lrand48 () % 10 >= 5)
      ushape_close (pr, o);
    o1 =
      subst ? ushape_substitute_texpr_array (ms, false, o, &d, &e, 1,
                                             NULL) :
      ushape_assign_texpr_array (ms, false, o, &d, &e, 1, NULL);
    FLAG (ms);
    RESULT (ushape_check (pr, o1));
    if (ushape_is_eq (ms, o, o1))
      RESULT ('x');
