                    ...
                    ap_linexpr0_t * lexpr, int *offsets)
{
  ...
  ap_linexpr0_t *exprn = shape_linexpr_of_node (lexpr, v2n, a->h->size,
                                                a->datadim, a->ptrdim);
#ifndef NDEBUG2
  fprintf (stdout, "%d:=", d);
                        ...
                        ap_linexpr0_t * lexpr, int *offsets)
{
  ...
  if (offsets != offsets)
    return NULL;                /* to remove warning on unsed arguments */
      ...
      if (!ushape_is_bottom (pr->man, bex))
        {
          ...
          if (!isfree)
            {
              ...
              // add to result
              rsize +=
                ushape_array_add (pr, true, array_r, 0, false, false, b);
              hgraph_free_internal (pr, h);
            }
          ...
          ap_dimperm_set_id (&perm);
          h =
            hgraph_node_expand (pr, a->h, i, NODE_VAR (a->h, i),
                                NODE_VAR_NEXT (a->h, i) + 1, &perm);
          b = ushape_alloc_internal (pr, a->datadim, a->ptrdim);
...
ushape_t *
ushape_substitute_x_y (ushape_internal_t * pr, ushape_t * a, size_t vx,
                       size_t vy)
{
  ap_dimperm_t perm;
  ap_dimperm_init (&perm, a->h->size);
...
ushape_assign_x_y_at_i (ushape_internal_t * pr,
                        ushape_t * a, size_t vx, size_t vy, size_t vi)
{
  // 
...
ushape_array_t *
ushape_substitute_x_next_null (ushape_internal_t * pr, bool isfree,
                               ushape_t * a, size_t vx)
{
  ...
  for (i = 0; i < a->h->size; i++)
    {
      ...
      ushape_t *b =
        ushape_alloc_internal (pr, a_copy->datadim, a_copy->ptrdim);
      b->h = hgraph_copy_internal (pr, h);
      hgraph_free_internal (pr, h);
  ...
  for (i = 0; i < a->h->size; i++)
    {
      ...
      hgraph_t *h =
        hgraph_node_add (pr, a_copy->h, i, vx, 1, &nx_succ, &perm);
      ushape_t *b =
        ushape_alloc_internal (pr, a_copy->datadim, a_copy->ptrdim);
      b->h = hgraph_copy_internal (pr, h);
      hgraph_free_internal (pr, h);
      ...
      if (!ushape_is_bottom (pr->man, bex))
        {
          ...
          ap_dimperm_set_id (&perm);
          h =
            hgraph_node_expand (pr, a_copy->h, i, NODE_VAR (a_copy->h, i),
                                NODE_VAR_NEXT (a_copy->h, i) + 1, &perm);
      ...
      else if (op->offx == OFFSET_NONE && op->op == PA_ASSIGN_PTR)
        {
          ...
          if (                  //IS_PTRDIM(op->info.ptr.y, a->datadim, a->ptrdim) &&
            ...
            {                   // y is not null
              ...
              else if (op->info.ptr.offy ==
                       ((int) (a->datadim + OFFSET_NEXT)))
                {               // x := y->next
                  // case (1): no new node
                  ...
                  if (nx != 0)
                  ...
                  else
                    {
                      ...
                      b = ushape_assign_x_y_at_i (pr, a, op->x,
                                                  ...
                                                  op->info.ptr.offy);
                      rsize +=
                        ushape_array_add (pr, true, r, 0, true, false, b);
                    }
                }
              else
                {
          ...
          else if (IS_NULLDIM (op->info.ptr.y) || ny == 0)
            {
  ...
  if (ushape_is_bottom (man, a))
  ...
  else
    {
      ...
      if (!destructive)
        ...
        b = a;
      op =
        shape_passign_of_linexpr_array (&d, &expr, 1, dim.intdim,
                                        dim.realdim);
#ifndef NDEBUG
                   ...
                   ushape_t * a, ap_dim_t * tdim, size_t size)
{
  ...
  if (pr->man_ucons >= pr->size_scons || pr->man_mset >= pr->size_scons)        /* TODO: may be improved */
    return r;
      ...
      if (n != NODE_NULL)
        {
          ...
          ap_linexpr0_t *lexpr =
            ap_linexpr0_alloc (AP_LINEXPR_DENSE, a->datadim + a->h->size);
  ...
  fflush (stdout);
  if (eqcons.p)
    ap_lincons0_array_fprint (stdout, &eqcons, NULL);
  else
    fprintf (stdout, "\nNULL");
            ...
            for (i = 0; i < size; i++)
              {
                ...
                ap_scalar_set_int (zero, OFFSET_MSET);
                ap_lincons0_t lcons =
                  ap_lincons0_make (AP_CONS_EQ, lexpr, zero);
#ifndef NDEBUG1
                fprintf (stdout,
                         "\n ti= %zu \n ++++ushape_strengthen: test constraints of type2 lcons=(",
                         ti);
...
ushape_substitute_actuals (ushape_internal_t * pr,
                           ushape_t * a, ap_dim_t * tdim, ap_dim_t * tnew,
                           size_t size)
{
  ushape_array_t *r;
  ushape_t *b;
...
#ifndef NDEBUG1
  fprintf (stdout, "\n====ushape_substitute_actuals: (%d) with subst=(",
           pr->meet_algo);
  for (i = 0; i < size; i++)
  ...
  if (size_data >= 1)
    {
      checked_malloc (tdim_data, ap_dim_t, sizeof (ap_dim_t), size_data,
                      return NULL;
        );
      checked_malloc (tnew_data, ap_dim_t, sizeof (ap_dim_t), size_data,
                      return NULL;
        );
    }
  else
  ...
  if (size_ptr >= 1)
    {
      checked_malloc (tdim_ptr, ap_dim_t, sizeof (ap_dim_t), size_ptr,
                      return NULL;
        );
      checked_malloc (tnew_ptr, ap_dim_t, sizeof (ap_dim_t), size_ptr,
                      return NULL;
        );
    }
  else
  ...
  if (size_ptr >= 1)
    {
      ...
      ap_dimperm_set_id (&perm);
      h =
        hgraph_substitute_array (pr, a->h, tdim_ptr, tnew_ptr, size_ptr,
                                 &perm);
  ...
  if (size_data >= 1)
    {
      ...
      checked_malloc (texpr_data, ap_texpr0_t *, sizeof (ap_texpr0_t *),
                      size_data, return NULL;
        );
      ...
      for (i = 0; i < pr->size_scons && !isbot; i++)
        {
          b->scons[i] =
            ap_abstract0_substitute_texpr_array (pr->man_scons[i], true,
                                                 b->scons[i], tdim_data,
                                                 texpr_data, size, NULL);
          isbot = ap_abstract0_is_bottom (pr->man_scons[i], b->scons[i]);
        }
      // free allocated data
      for (i = 0; i < size_data; i++)
  ...
  if (op->x < a->datadim || op->op == PA_ASSIGN_INT)
    ...
    {                           // pure pointer substitution
      ...
      if (op->offx == OFFSET_NONE)
                ...
                if (            //IS_PTRDIM(op->info.ptr.y, a->datadim, a->ptrdim) &&
                  ...
                  {             // x := null
                    if (nx == 0)
                      {
                        b_array =
                          ushape_substitute_x_null (pr,
                                                    !IS_NULLDIM (op->info.ptr.
                                                                 y), a, vx);
                        ushape_array_add_array (pr, true, r, b_array);
                      }
                    else
                      {
                        ERROR
                          ("Bad substitution: not an allowed right value!",
                           return NULL;
                          );
                      }
                  }
          ...
          if (nx == 0)
            {
              ...
              //        );
              FEEDBACK ("Bad substitution: null pointer dereference!",
                        return NULL;
            )}

          size_t vy = DIM2PTR (op->info.ptr.y, a->datadim);
          if (IS_NULLDIM (op->info.ptr.y) && op->info.ptr.offy == OFFSET_NONE)
            {                   // x->next = null
        ...
        {                       // TODO: other pointer fields than next
          ERROR ("Bad substitution: not an elementary substitution!",;);
        }
    }
  if (r && r->size)
  ...
  if (ushape_is_bottom (man, a))
  ...
  else
    {
      ...
      if (!destructive)
        ...
        b = a;
      op =
        shape_passign_of_linexpr_array (tdim, texpr, size, dim.intdim,
                                        dim.realdim);
#ifndef NDEBUG
      fprintf (stdout,
                           ...
                           ap_texpr0_t ** texpr, size_t size, ushape_t * dest)
{
  if (ushape_is_bottom (man, a))
  ...
  else
    {
      ...
      ushape_array_t *r;
      ushape_internal_t *pr =
        ushape_init_from_manager (man, AP_FUNID_ASSIGN_TEXPR_ARRAY, 0);
      ap_dimension_t dim = ushape_dimension (man, a);
      if (!destructive)
        b = ushape_copy_internal (pr, a);
      else
        b = a;
      op =
        shape_passign_of_texpr_array (tdim, texpr, size, dim.intdim,
                                      dim.realdim);
#ifndef NDEBUG
      fprintf (stdout,
