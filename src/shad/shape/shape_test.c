...
void
print_shape (const char *msg, shape_t * a)
{
...
test_misc (void)
{
  ...
  if (shape_check (pr, bot) != '.')
    printf ("shape_bottom failed #0\n");
  if (shape_check (pr, top) != '.')
    printf ("shape_top failed #0\n");
  if (d1.intdim != intdim || d1.realdim != realdim)
    printf ("shape_dimension failed #1\n");
  if (d2.intdim != intdim || d2.realdim != realdim)
    printf ("shape_dimension failed #2\n");
  if (!shape_is_bottom (ms, bot))
    printf ("shape_is_bottom failed #3\n");
  if (shape_is_bottom (ms, top))
    printf ("shape_is_bottom failed #4\n");
  if (shape_is_top (ms, bot))
    printf ("shape_is_top failed #5\n");
  if (!shape_is_top (ms, top))
    printf ("shape_is_top failed #6\n");
  if (!shape_is_leq (ms, bot, top))
    printf ("shape_is_leq failed #7\n");
  if (shape_is_leq (ms, top, bot))
    printf ("shape_is_leq failed #8\n");
  if (!shape_is_eq (ms, bot, bot))
    printf ("shape_is_eq failed #9\n");
  if (!shape_is_eq (ms, top, top))
    printf ("shape_is_eq failed #10\n");
  if (shape_is_eq (ms, bot, top))
    printf ("shape_is_eq failed #11\n");
  ...
  for (i = 0; i < 5; i++)
    {
      ...
      if (d.intdim < intdim || d.realdim != realdim)
        printf ("shape_dimension failed #18\n");
      if (!shape_is_leq (ms, bot, o))
        printf ("shape_is_leq failed #19\n");
      if (!shape_is_leq (ms, o, top))
        printf ("shape_is_leq failed #20\n");
      if (!shape_is_eq (ms, o, c))
        printf ("shape_is_eq failed #21\n");
      shape_size (ms, o);
      shape_close (pr, o);
      // not implemented
      //shape_minimize(ms,o);
  ...
  for (i = 0; i < N; i++)
    {
      ...
      ap_dimension_t d = shape_dimension (ms, o);
      if (d.intdim < intdim || d.realdim != realdim)
        printf ("shape_dimension failed #14\n");
      if (!shape_is_leq (ms, bot, o))
        printf ("shape_is_leq failed #15\n");
      if (!shape_is_leq (ms, o, top))
        printf ("shape_is_leq failed #16\n");
      if (!shape_is_eq (ms, o, c))
        printf ("shape_is_eq failed #17\n");
      shape_size (ms, o);
      shape_close (pr, o);
      // not implemented
      //shape_minimize(ms,o);
    ...
    for (i = 0; i < nb; i++)
      {
        t.p[i] = ap_lincons0_make ((lrand48 () % 100 >= 90) ? AP_CONS_EQ :
                                   ...
                                   AP_CONS_SUPEQ,
                                   shape_linexpr_random (pr, mode, intdim,
                                                         realdim), NULL);
        tt.p[i] = random_from_lincons (t.p[i]);
      }
    o = shape_top (ms, intdim, realdim);
    oo = shape_meet_lincons_array (ms, true, o, &t);
    FLAG (ms);
    RESULT (shape_check (pr, o));
    if (shape_is_eq (ms, o, oo))
      RESULT ('x');
    if (shape_is_leq (ms, oo, o))
      ERROR_RESULT ("best flag");
    shape_free (ms, o);
    shape_free (ms, oo);
    ap_lincons0_array_clear (&t);
    ap_lincons0_array_clear (&tt);
  }
  ENDLOOP;
}
...
test_meet_array (void)
{
  ...
  LOOP
  {
    ...
    oo = shape_meet_array (ms, o, NB_MEET);
    FLAG (ms);
    RESULT (shape_check (pr, oo));
    for (i = 0; i < NB_MEET; i++)
      if (!shape_is_leq (ms, oo, o[i]))
        ERROR_RESULT ("not lower bound");
    for (i = 0; i < NB_MEET; i++)
      shape_free (ms, o[i]);
    shape_free (ms, oo);
...
test_add_pcons_once (exprmode_t mode, shape_t * o, pcons0_array_t * array)
{
  ...
  o2 = shape_meet_pcons_array (pr, true, o1, array);
  FLAG (ms);
  shape_check (pr, o);
  if (o2)
    {
...
test_add_tcons (exprmode_t mode)
{
  ...
  if (!shape_is_eq (ms, or, om))
    printf ("shape_meet_tcons failed #0\n");
  shape_free (ms, o);
  shape_free (ms, or);
  shape_free (ms, om);
  ...
  if (!shape_is_eq (ms, or, om))
    printf ("shape_meet_tcons failed #1\n");
  shape_free (ms, o);
  shape_free (ms, om);
  shape_free (ms, or);
  ...
  if (!shape_is_eq (ms, or, om))
    printf ("shape_meet_tcons failed #2\n");
  shape_free (ms, o);
  shape_free (ms, om);
  shape_free (ms, or);
  ...
  if (!shape_is_eq (ms, or, om))
    printf ("shape_meet_tcons failed #3\n");
  shape_free (ms, o);
  shape_free (ms, om);
  shape_free (ms, or);
  ...
  om = shape_make (pr, 2, intdim, realdim);
  if (!shape_is_eq (ms, or, om))
    printf ("shape_meet_tcons failed #4\n");
  shape_free (ms, o);
  shape_free (ms, om);
  shape_free (ms, or);
...
test_assign (int subst, exprmode_t mode)
{
  ...
  if (!shape_is_eq (ms, o, or))
    printf ("shape_assign_linexpr_array failed #1\n");
  ap_linexpr0_free (l);
  shape_free (ms, or);
  ...
  RESULT (shape_check (pr, or));
  if (shape_is_eq (ms, o, or))
    printf ("shape_assign_linexpr_array failed #2\n");
  ap_linexpr0_free (l);
  shape_free (ms, o);
...
test_assign_texpr (int subst, exprmode_t mode)
{
  ...
  RESULT (shape_check (pr, or));
  if (!shape_is_eq (ms, o, or))
    printf ("shape_assign_texpr_array failed #1\n");
  shape_free (ms, or);
  ...
  t3 =
    shape_texpr_x_y_v_cst_data (1, DIM_X + 1, 0, 0, 0, 0, 2, true, intdim,
                                realdim);
  or = shape_assign_texpr_array (ms, false, o, &d, &t3, 1, NULL);
  FLAG (ms);
  RESULT (shape_check (pr, o));
  if (shape_is_eq (ms, o, or))
    printf ("shape_assign_texpr_array failed #3\n");
  //ap_texpr0_free(t3);
  shape_free (ms, o);
  o = or;
  or = NULL;
  ...
  RESULT (shape_check (pr, o));
  if (shape_is_eq (ms, o, or))
    printf ("shape_assign_texpr_array failed #4\n");
  //ap_texpr0_free(t4);
  shape_free (ms, o);
  o = or;
  or = NULL;
  ...
  RESULT (shape_check (pr, or));
  if (shape_is_eq (ms, o, or))
    printf ("shape_assign_texpr_array failed #5\n");
  shape_free (ms, o);
  o = or;
  or = NULL;
  ...
  RESULT (shape_check (pr, or));
  if (shape_is_eq (ms, o, or))
    printf ("shape_assign_texpr_array failed #6\n");
  shape_free (ms, o);
  o = or;
  or = NULL;
...
test_prg_add2 (void)
{
  shape_t *top, *l6, *l7, *l8, *l9, *l10, *l11, *l12, *l13, *l14, *l15,
    *ljoin, *lwiden, *l16;
  ...
  whileCond.p[0] =
    shape_tcons_diff_x_y (xi, false, NULL_DIM, false, intdim, realdim);
  notWhileCond = ap_tcons0_array_make (1);
  notWhileCond.p[0] =
    shape_tcons_same_x_y (xi, false, NULL_DIM, false, intdim, realdim);
  t10 =
    shape_texpr_x_y_v_cst_data (1, xi, 0, 0, 0, 0, 2, true, intdim, realdim);
  ...
  // init with 0=0
  init0.p[0] =
    ap_tcons0_make (AP_CONS_EQ, ap_texpr0_cst_scalar_int (0), NULL);
  shape_approximate (ms, top, 1);
  l6 = shape_meet_tcons_array (ms, false, top, &init0);
  ap_tcons0_clear (init0.p);
  init = ap_tcons0_array_make (1);
  init.p[0] =
    shape_tcons_reach_x_y (DIM_X, false, NULL_DIM, false, intdim, realdim);
  shape_approximate (ms, top, 1);
  ...
  if (code <= 1)
    {
      whileCond = ap_tcons0_array_make (1);
      whileCond.p[0] =
        shape_tcons_diff_x_y (xi, 0, NULL_DIM, 0, intdim, realdim);
    }
  else
    {
      ...
      whileCond.p[0] =
        shape_tcons_diff_x_y (xi, 0, NULL_DIM, 0, intdim, realdim);
      whileCond.p[1] =
        shape_tcons_diff_x_y (yi, 0, NULL_DIM, 0, intdim, realdim);
    }
  notWhileCond = ap_tcons0_array_make (1);
  notWhileCond.p[0] =
    shape_tcons_same_x_y (xi, 0, NULL_DIM, 0, intdim, realdim);
  t12 =
    shape_texpr_x_y_v_cst_data (1, xi, 0, 0, 0, 0, 2, true, intdim, realdim);
...
void
test_prg_fstNot0 (void)
{
  shape_t *l5, *l7, *l8, *l9, *l10, *l11, *l11b, *l12, *l13, *l14, *l15,
    *ljoin, *lwiden, *l16;
  ap_tcons0_array_t whileCond, notWhileCond, ifCond, notIfCond;
  ap_dim_t x, xi, y, z;
  ...
  ifCond.p[0] = ap_tcons0_make (AP_CONS_DISEQ,
                                shape_texpr_x_y_v_cst_data (1, xi, 0, 0, 0, 0,
                                                            0, false, intdim,
                                                            realdim), NULL);
  notWhileCond = ap_tcons0_array_make (1);
  notWhileCond.p[0] =
    shape_tcons_diff_x_y (y, 0, NULL_DIM, 0, intdim, realdim);
  notIfCond = ap_tcons0_array_make (1);
  notIfCond.p[0] = ap_tcons0_make (AP_CONS_EQ,
                                   shape_texpr_x_y_v_cst_data (1, xi, 0, 0, 0,
                                                               0, 0, false,
                                                               intdim,
                                                               realdim),
                                   NULL);
...
void
test_prg_getMax (void)
{
  shape_t *l5, *l7, *l8, *l9, *l10, *l11, *l12b, *l12, *l13, *l14, *l15, *l16,
    *ljoin, *lwiden, *l17;
  ap_tcons0_array_t whileCond, notWhileCond, ifCond, notIfCond;
  ap_dim_t x, xi, y, m;
  ...
  m = intdim - 1;
  t7 =
    shape_texpr_x_y_v_cst_data (1, x, 0, 0, 0, 0, 0, false, intdim, realdim);
  t8 = shape_texpr_x_n_next (DIM_X, 0, intdim, realdim);        /* x */
  whileCond = ap_tcons0_array_make (1);
  whileCond.p[0] = shape_tcons_diff_x_y (xi, 0, NULL_DIM, 0, intdim, realdim);
  ifCond = ap_tcons0_array_make (1);
  ifCond.p[0] = ap_tcons0_make (AP_CONS_SUPEQ,
                                shape_texpr_x_y_v_cst_data (1, xi, 0, 0, -1,
                                                            m, -1, false,
                                                            intdim, realdim),
                                NULL);
  notWhileCond = ap_tcons0_array_make (1);
  ...
  notIfCond.p[0] = ap_tcons0_make (AP_CONS_SUPEQ,
                                   shape_texpr_x_y_v_cst_data (-1, xi, 0, 0,
                                                               1, m, 0, false,
                                                               intdim,
                                                               realdim),
                                   NULL);
...
test_prg_initMod2 (void)
{
  ...
  ifCond = ap_tcons0_array_make (1);
  ifCond.p[0] = ap_tcons0_make (AP_CONS_SUPEQ,
                                shape_texpr_x_y_v_cst_data (0, 0, 0, 0, -1, k,
                                                            0, false, intdim,
                                                            realdim), NULL);
  notWhileCond = ap_tcons0_array_make (1);
  notWhileCond.p[0] =
  ...
  notIfCond = ap_tcons0_array_make (1);
  notIfCond.p[0] = ap_tcons0_make (AP_CONS_SUPEQ,
                                   shape_texpr_x_y_v_cst_data (0, 0, 0, 0, 1,
                                                               k, -1, false,
                                                               intdim,
                                                               realdim),
                                   NULL);
...
void
test_prg_delAllGeV (void)
{
  shape_t *l6, *l8, *l9, *l10, *l11, *l12, *l13, *l14, *l15, *l16,
    *l17, *l18, *l19, *l20, *l21, *l22, *l23, *l24, *l25, *l26, *l27, *l28,
    *l29, *l30, *ljoin, *lwiden, *l31;
  ap_tcons0_array_t whileCond, notWhileCond, ifCond1, notIfCond1, ifCond2,
    notIfCond2;
  ap_dim_t x, xi, y, z, v;
  ap_texpr0_t *t7, *t8, *t11, *t13, *t18, *t19, *t21, *t26, *t28;
  size_t i;
  bool fin = false;
