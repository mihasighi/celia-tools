  ...
  else if (ofs >= ((int) intdim))
    {
      ...
      switch (ofs - intdim)
        ...
        case OFFSET_PREV:
          ...
          break;
        default:
          fprintf (stream, " \\fld%zu(", ofs - intdim);
          break;
        }
      shape_dim_fprint (stream, dim);
      fprintf (stream, ") ");
...
void
shape_offsets_fprint (FILE * stream, int *offsets, size_t intdim,
                      size_t ptrdim)
{
#ifndef NDEBUG
      ...
      if (c->type == DATA_CONS)
        {
          ...
          shape_offsets_fprint (stream, c->info.data.offsets, c->intdim,
                                c->ptrdim);
        }
      if (c->type == SL3_CONS)
        {
        ...
        {
          shape_offset_fprint (stream, c->info.ptr.offx, c->intdim,
                               c->info.ptr.x);
          switch (c->type)
            {
            case EQ_CONS:
